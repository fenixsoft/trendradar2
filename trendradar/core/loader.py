...
def _load_smzdm_config(config_data: Dict) -> Dict:
    """加载 smzdm（什么值得买）数码好价抓取配置"""
    smzdm = config_data.get("smzdm", {})
    return {
        "ENABLED": smzdm.get("enabled", False),
        "CATEGORY_URL": smzdm.get("category_url", "https://www.smzdm.com/fenlei/diannaoshuma/h5c1s0f0t0p1/"),
        "MAX_ITEMS": smzdm.get("max_items", 50),
        "MAX_AGE_HOURS": smzdm.get("max_age_hours", 24),
        "HEADLESS": smzdm.get("headless", True),
        "SCROLL_ROUNDS": smzdm.get("scroll_rounds", 6),
    }


def _load_aggregation_config(config_data: Dict) -> Dict:
    """加载聚合存储推送渠道配置"""
    agg = config_data.get("aggregation", {})
    storage_push = agg.get("storage_push", {})
    return {
        "STORAGE_PUSH": {
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    # 聚合渠道配置（smzdm 等）
    config["SMZDM"] = _load_smzdm_config(config_data)

    # 存储推送渠道配置
    config["AGGREGATION"] = _load_aggregation_config(config_data)
    ...
    # 从 COS 读取面板写入的 AI 配置 / 设置并覆盖（仅在配置了远程存储时生效）
    try:
        from trendradar.storage.push import apply_remote_config_overrides
        apply_remote_config_overrides(config)
    except Exception as e:  # noqa: BLE001
        print(f"[配置] 读取 COS 面板配置失败（忽略，使用本地配置）: {e}")
