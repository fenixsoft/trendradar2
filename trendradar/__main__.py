...
from trendradar.crawler.smzdm import fetch_digital_deals
from trendradar.storage import convert_crawl_results_to_news_data
from trendradar.storage.push import publish_latest
...
class NewsAnalyzer:
    ...
    def _publish_to_storage(
        ...
        try:
            ...
            agg = config.get("AGGREGATION", {}).get("STORAGE_PUSH", {})
            ...
            if smzdm_cfg.get("ENABLED"):
                result = fetch_digital_deals(
                    url=smzdm_cfg.get("CATEGORY_URL"),
                    max_age_hours=smzdm_cfg.get("MAX_AGE_HOURS", 24),
                    max_items=smzdm_cfg.get("MAX_ITEMS", 50),
                    headless=smzdm_cfg.get("HEADLESS", True),
                    scroll_rounds=smzdm_cfg.get("SCROLL_ROUNDS", 6),
                )
                channels.append({
            ...
            if ext.get("SUMMARY") and ai_config.get("ENABLED"):
                ...
                for ch in channels:
                    if ch.get("items"):
                        ch["items"] = generate_chinese_summaries(
                            ch["items"], ai_config,
                            source_summary=True, fallback_from_title=True,
                        )
            ...
            err = publish_latest(config, payload)
            if err:
                print(f"[存储推送] {err}")
            else:
                total = sum(len(c.get("items", [])) for c in channels)
                print(f"[存储推送] 已写入 latest.json，共 {total} 条")
